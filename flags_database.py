...
def closest_color(color, palette):
    min_colours = {}
    # if type(palette == str):
    #     selection = list(map(webcolors.hex_to_rgb, palette['hex_to_name'].keys()))
    ...
    for option in palette:
        ...
        gd = (g_c - color[1]) ** 2
        bd = (b_c - color[2]) ** 2
        min_colours[(rd + gd + bd)] = option
    converted = min_colours[min(min_colours.keys())]
...
class Flag:
    ...
    def base_colors(self, resize = 10):
        # Resize image to speed up processing
        img = self.base_image.copy()
        if smooth:
            img = img.filter(ImageFilter.SMOOTHMORE)
        if resize:
        ...
        # if self.name == 'queer':
        #     colors_rgb = [(0, 0, 0), (153, 216, 235), (2, 164, 236), (183, 229, 32), (255, 255, 255), (255, 200, 12), (252, 102, 103), (254, 174, 201)]
        #     conversion = {}
        #     for color in paletted.getcolors():
        #         conversion[color] = closest_color(color, colors_rgb)
        #     pixels = paletted.load()
        #     for x in range(paletted.size[0]):
    ...
    def derive_palettes(self):
        ...
        for palette in palettes.keys():
            for base_rgb in palette_conversion['base']:
                ...
                _rgb = closest_color(base_rgb, list(map(webcolors.hex_to_rgb, palettes['xkcd']['hex_to_name'].keys())))
    ...
    def raw_palette(self, save = False):
        img = self.base_image
...
def flag_dataframe(flags, palette = 'xkcd', medium = 'Marker', progress = False):
...
# def all_flags(flags, palette = 'xkcd', medium = 'Marker', progress = True):
#     df = flag_dataframe(flags[0], palette = palette, medium = medium, progress = progress)
#     for flag in flags[1:]:
#         df.append(flag_dataframe(flag,palette = palette, medium = medium, progress = progress))
